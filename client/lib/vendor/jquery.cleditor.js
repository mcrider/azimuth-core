...
(function ($) {
  ...
  $.cleditor = {
    defaultOptions: {
      ...
      controls: 'source | bold italic underline strikethrough | font size style color removeformat | bullets numbering | outdent indent | alignleft center alignright justify | link unlink | cut copy paste pastetext ',
  ...
  var
    // Misc constants
    BACKGROUND_COLOR = 'backgroundColor', BUTTON = 'button', BUTTON_NAME = 'buttonName', CHANGE = 'change', CLEDITOR = 'cleditor', CLICK = 'click', DISABLED = 'disabled', DIV_TAG = '<div>', TRANSPARENT = 'transparent', UNSELECTABLE = 'unselectable',
    // Class name constants
    MAIN_CLASS = 'cleditorMain',
    // main containing div
    TOOLBAR_CLASS = 'cleditorToolbar',
    // toolbar div inside main div
    GROUP_CLASS = 'cleditorGroup',
    // group divs inside the toolbar div
    BUTTON_CLASS = 'cleditorButton',
    // button divs inside group div
    DISABLED_CLASS = 'cleditorDisabled',
    // disabled button divs
    DIVIDER_CLASS = 'cleditorDivider',
    // divider divs inside group div
    POPUP_CLASS = 'cleditorPopup',
    // popup divs inside body
    LIST_CLASS = 'cleditorList',
    // list popup divs inside body
    COLOR_CLASS = 'cleditorColor',
    // color popup div inside body
    PROMPT_CLASS = 'cleditorPrompt',
    // prompt popup divs inside body
    MSG_CLASS = 'cleditorMsg',
    // message popup div inside body
    // Browser detection
    ua = navigator.userAgent.toLowerCase(), ie = /msie/.test(ua), ie6 = /msie\s6/.test(ua), webkit = /webkit/.test(ua),
  ...
  function buttonClick(e) {
    ...
    if (buttonName == 'source') {
      ...
      if (popupName) {
        ...
        editor.$frame[0].contentWindow.print();  // All other buttons
  ...
  function hoverEnter(e) {
    ...
    $div.css(BACKGROUND_COLOR, $div.data(BUTTON_NAME) ? '#a8abae' : '#a8abae');
  }
  ...
  function refresh(editor) {
    ...
    doc.open();
    doc.write(options.docType + '<html>' + (options.docCSSFile === '' ? '' : '<head><link rel="stylesheet" type="text/css" href="' + options.docCSSFile + '" /></head>') + '<body style="' + options.bodyStyle + '"></body></html>');
    doc.close();
    ...
    if (ie) {
      ...
      $doc.bind('beforedeactivate beforeactivate selectionchange keypress', function (e) {
        ...
        if (e.type == 'beforedeactivate')
          editor.inactive = true;  // Get rid of the bogus selection and flag the editor as active
    ...
    $(function () {
      var $toolbar = editor.$toolbar, $group = $toolbar.children('div:last'), wid = $main.width();
      // Resize the toolbar
